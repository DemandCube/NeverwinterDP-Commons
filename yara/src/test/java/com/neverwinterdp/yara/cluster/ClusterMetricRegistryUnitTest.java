...
public class ClusterMetricRegistryUnitTest {
  ...
  public void testBasic() throws IOException {
    ...
    for(int i = 0; i < 1000; i++) {
      ...
      if(duration % 3 == 0) {
        server1.getCounter("counter").incr() ;
        server1.getTimer("timer").update(duration, TimeUnit.NANOSECONDS);
      } else {
        server2.getCounter("counter").incr() ;
        server2.getTimer("timer").update(duration, TimeUnit.NANOSECONDS);
      }
    }
