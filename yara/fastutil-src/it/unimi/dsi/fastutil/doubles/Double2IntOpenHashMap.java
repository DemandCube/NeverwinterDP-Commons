...
public class Double2IntOpenHashMap extends AbstractDouble2IntMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Integer get( final Double ok ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Double.doubleToLongBits(key[ pos ]) == Double.doubleToLongBits( k) ) ) return (Integer.valueOf(value[ pos ]));
   pos = ( pos + 1 ) & mask;
  }
  return (null);
 }
 ...
 private final class MapEntrySet extends AbstractObjectSet<Double2IntMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   // There's always an unused entry.
   while( used[ pos ] ) {
    if ( ( Double.doubleToLongBits(key[ pos ]) == Double.doubleToLongBits(k) ) ) return ( (value[ pos ]) == (((e.getValue()).intValue())) );
    pos = ( pos + 1 ) & mask;
   }
   return false;
  }
