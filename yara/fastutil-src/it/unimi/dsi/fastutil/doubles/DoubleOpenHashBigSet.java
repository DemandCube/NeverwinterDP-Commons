...
public class DoubleOpenHashBigSet extends AbstractDoubleSet implements java.io.Serializable, Cloneable, Hash, Size64 {
 ...
 public boolean add( final double k ) {
  final long h = it.unimi.dsi.fastutil.HashCommon.murmurHash3( Double.doubleToRawLongBits(k) ^ mask );
  // The starting point.
  int displ = (int)( h & segmentMask );
  int base = (int)( ( h & mask ) >>> BigArrays.SEGMENT_SHIFT );
  // There's always an unused entry.
  while( used[ base ][ displ ] ) {
   if ( ( Double.doubleToLongBits(key[ base ][ displ ]) == Double.doubleToLongBits(k) ) ) return false;
 ...
 protected final long shiftKeys( long pos ) {
  ...
  for(;;) {
   ...
   while( BooleanBigArrays.get( used, pos ) ) {
    slot = it.unimi.dsi.fastutil.HashCommon.murmurHash3( Double.doubleToRawLongBits(DoubleBigArrays.get( key, pos )) ^ mask ) & mask;
 ...
 public boolean remove( final double k ) {
  ...
  // There's always an unused entry.
  while( used[ base ][ displ ] ) {
   if ( ( Double.doubleToLongBits(key[ base ][ displ ]) == Double.doubleToLongBits(k) ) ) {
    size--;
 ...
 public boolean contains( final double k ) {
  ...
  // There's always an unused entry.
  while( used[ base ][ displ ] ) {
   if ( ( Double.doubleToLongBits(key[ base ][ displ ]) == Double.doubleToLongBits(k) ) ) return true;
   base = ( base + ( ( displ = ( displ + 1 ) & segmentMask ) == 0 ? 1 : 0 ) ) & baseMask;
  }
  return false;
 }
 ...
 protected void rehash( final long newN ) {
  ...
  for( long i = size; i-- != 0; ) {
   ...
   k = key[ base ][ displ ];
   h = it.unimi.dsi.fastutil.HashCommon.murmurHash3( Double.doubleToRawLongBits(k) ^ mask );
   // The starting point.
