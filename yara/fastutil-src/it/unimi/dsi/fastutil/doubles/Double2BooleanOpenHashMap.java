...
public class Double2BooleanOpenHashMap extends AbstractDouble2BooleanMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public boolean put(final double k, final boolean v) {
  // The starting point.
  int pos = (int)it.unimi.dsi.fastutil.HashCommon.murmurHash3( Double.doubleToRawLongBits(k) ^ mask ) & mask;
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Double.doubleToLongBits(key[ pos ]) == Double.doubleToLongBits(k) ) ) {
 ...
 protected final int shiftKeys( int pos ) {
  ...
  for(;;) {
   pos = ( ( last = pos ) + 1 ) & mask;
   while( used[ pos ] ) {
    slot = (int)it.unimi.dsi.fastutil.HashCommon.murmurHash3( Double.doubleToRawLongBits(key[ pos ]) ^ mask ) & mask;
 ...
 public Boolean get( final Double ok ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Double.doubleToLongBits(key[ pos ]) == Double.doubleToLongBits( k) ) ) return (Boolean.valueOf(value[ pos ]));
   pos = ( pos + 1 ) & mask;
  }
  return (null);
 }
 ...
 public boolean get( final double k ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Double.doubleToLongBits(key[ pos ]) == Double.doubleToLongBits(k) ) ) return value[ pos ];
   pos = ( pos + 1 ) & mask;
  }
  return defRetValue;
 }
 ...
 public boolean containsKey( final double k ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Double.doubleToLongBits(key[ pos ]) == Double.doubleToLongBits(k) ) ) return true;
   pos = ( pos + 1 ) & mask;
  }
  return false;
 }
 ...
 private class MapIterator {
  ...
  public int nextEntry() {
   ...
   if ( pos < 0 ) {
    ...
    // There's always an unused entry.
    while( used[ pos ] ) {
     if ( ( Double.doubleToLongBits(key[ pos ]) == Double.doubleToLongBits(k) ) ) return pos;
     pos = ( pos + 1 ) & mask;
    }
   }
   last = pos;
 ...
 private final class MapEntrySet extends AbstractObjectSet<Double2BooleanMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   // There's always an unused entry.
   while( used[ pos ] ) {
    if ( ( Double.doubleToLongBits(key[ pos ]) == Double.doubleToLongBits(k) ) ) return ( (value[ pos ]) == (((e.getValue()).booleanValue())) );
    pos = ( pos + 1 ) & mask;
   }
   return false;
  }
 ...
 protected void rehash( final int newN ) {
  ...
  for( int j = size; j-- != 0; ) {
   while( ! used[ i ] ) i++;
   k = key[ i ];
   pos = (int)it.unimi.dsi.fastutil.HashCommon.murmurHash3( Double.doubleToRawLongBits(k) ^ mask ) & mask;
