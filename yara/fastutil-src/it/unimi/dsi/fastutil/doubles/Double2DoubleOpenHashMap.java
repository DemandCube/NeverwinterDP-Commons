...
public class Double2DoubleOpenHashMap extends AbstractDouble2DoubleMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Double get( final Double ok ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Double.doubleToLongBits(key[ pos ]) == Double.doubleToLongBits( k) ) ) return (Double.valueOf(value[ pos ]));
   pos = ( pos + 1 ) & mask;
  }
  return (null);
 }
 ...
 private final class MapEntrySet extends AbstractObjectSet<Double2DoubleMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   // There's always an unused entry.
   while( used[ pos ] ) {
    if ( ( Double.doubleToLongBits(key[ pos ]) == Double.doubleToLongBits(k) ) ) return ( (value[ pos ]) == (((e.getValue()).doubleValue())) );
    pos = ( pos + 1 ) & mask;
   }
   return false;
  }
