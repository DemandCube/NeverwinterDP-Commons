...
public class Object2ByteOpenCustomHashMap <K> extends AbstractObject2ByteMap <K> implements java.io.Serializable, Cloneable, Hash {
 ...
 public byte put(final K k, final byte v) {
  // The starting point.
  int pos = ( it.unimi.dsi.fastutil.HashCommon.murmurHash3( strategy.hashCode( (K) (k)) ^ mask ) ) & mask;
 ...
 protected final int shiftKeys( int pos ) {
  ...
  for(;;) {
   pos = ( ( last = pos ) + 1 ) & mask;
   while( used[ pos ] ) {
    slot = ( it.unimi.dsi.fastutil.HashCommon.murmurHash3( strategy.hashCode( (K) (key[ pos ])) ^ mask ) ) & mask;
 ...
 protected void rehash( final int newN ) {
  ...
  for( int j = size; j-- != 0; ) {
   while( ! used[ i ] ) i++;
   k = key[ i ];
   pos = ( it.unimi.dsi.fastutil.HashCommon.murmurHash3( strategy.hashCode( (K) (k)) ^ mask ) ) & mask;
