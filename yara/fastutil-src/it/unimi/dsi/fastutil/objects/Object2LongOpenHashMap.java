...
public class Object2LongOpenHashMap <K> extends AbstractObject2LongMap <K> implements java.io.Serializable, Cloneable, Hash {
 ...
 protected final int shiftKeys( int pos ) {
  ...
  for(;;) {
   pos = ( ( last = pos ) + 1 ) & mask;
   while( used[ pos ] ) {
    slot = ( (key[ pos ]) == null ? 0x87fcd5c : it.unimi.dsi.fastutil.HashCommon.murmurHash3( (key[ pos ]).hashCode() ^ mask ) ) & mask;
 ...
 protected void rehash( final int newN ) {
  ...
  for( int j = size; j-- != 0; ) {
   while( ! used[ i ] ) i++;
   k = key[ i ];
   pos = ( (k) == null ? 0x87fcd5c : it.unimi.dsi.fastutil.HashCommon.murmurHash3( (k).hashCode() ^ mask ) ) & mask;
