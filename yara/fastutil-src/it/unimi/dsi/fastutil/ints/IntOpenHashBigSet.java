...
public class IntOpenHashBigSet extends AbstractIntSet implements java.io.Serializable, Cloneable, Hash, Size64 {
 ...
 public boolean add( final int k ) {
  final long h = ( it.unimi.dsi.fastutil.HashCommon.murmurHash3( (long)( (k) ^ mask ) ) );
  // The starting point.
 ...
 protected final long shiftKeys( long pos ) {
  ...
  for(;;) {
   ...
   while( BooleanBigArrays.get( used, pos ) ) {
    slot = ( it.unimi.dsi.fastutil.HashCommon.murmurHash3( (long)( (IntBigArrays.get( key, pos )) ^ mask ) ) ) & mask;
 ...
 protected void rehash( final long newN ) {
  ...
  for( long i = size; i-- != 0; ) {
   ...
   k = key[ base ][ displ ];
   h = ( it.unimi.dsi.fastutil.HashCommon.murmurHash3( (long)( (k) ^ mask ) ) );
   // The starting point.
