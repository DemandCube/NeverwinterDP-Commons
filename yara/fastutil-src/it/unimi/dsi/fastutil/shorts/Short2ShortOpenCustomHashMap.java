...
public class Short2ShortOpenCustomHashMap extends AbstractShort2ShortMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public short put(final short k, final short v) {
  // The starting point.
  int pos = ( it.unimi.dsi.fastutil.HashCommon.murmurHash3( strategy.hashCode(k) ^ mask ) ) & mask;
 ...
 protected final int shiftKeys( int pos ) {
  ...
  for(;;) {
   pos = ( ( last = pos ) + 1 ) & mask;
   while( used[ pos ] ) {
    slot = ( it.unimi.dsi.fastutil.HashCommon.murmurHash3( strategy.hashCode(key[ pos ]) ^ mask ) ) & mask;
 ...
 protected void rehash( final int newN ) {
  ...
  for( int j = size; j-- != 0; ) {
   while( ! used[ i ] ) i++;
   k = key[ i ];
   pos = ( it.unimi.dsi.fastutil.HashCommon.murmurHash3( strategy.hashCode(k) ^ mask ) ) & mask;
