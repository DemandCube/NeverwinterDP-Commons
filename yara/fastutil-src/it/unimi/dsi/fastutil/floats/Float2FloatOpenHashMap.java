...
public class Float2FloatOpenHashMap extends AbstractFloat2FloatMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Float get( final Float ok ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits( k) ) ) return (Float.valueOf(value[ pos ]));
   pos = ( pos + 1 ) & mask;
  }
  return (null);
 }
 ...
 private final class MapEntrySet extends AbstractObjectSet<Float2FloatMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   // There's always an unused entry.
   while( used[ pos ] ) {
    if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits(k) ) ) return ( (value[ pos ]) == (((e.getValue()).floatValue())) );
    pos = ( pos + 1 ) & mask;
   }
   return false;
  }
