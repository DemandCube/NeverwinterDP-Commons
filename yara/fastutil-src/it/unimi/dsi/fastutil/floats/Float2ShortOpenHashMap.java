...
public class Float2ShortOpenHashMap extends AbstractFloat2ShortMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Short get( final Float ok ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits( k) ) ) return (Short.valueOf(value[ pos ]));
   pos = ( pos + 1 ) & mask;
  }
  return (null);
 }
 ...
 private final class MapEntrySet extends AbstractObjectSet<Float2ShortMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   // There's always an unused entry.
   while( used[ pos ] ) {
    if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits(k) ) ) return ( (value[ pos ]) == (((e.getValue()).shortValue())) );
    pos = ( pos + 1 ) & mask;
   }
   return false;
  }
