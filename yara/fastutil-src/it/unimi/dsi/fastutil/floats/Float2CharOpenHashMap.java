...
public class Float2CharOpenHashMap extends AbstractFloat2CharMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Character get( final Float ok ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits( k) ) ) return (Character.valueOf(value[ pos ]));
   pos = ( pos + 1 ) & mask;
  }
  return (null);
 }
 ...
 private final class MapEntrySet extends AbstractObjectSet<Float2CharMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   // There's always an unused entry.
   while( used[ pos ] ) {
    if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits(k) ) ) return ( (value[ pos ]) == (((e.getValue()).charValue())) );
    pos = ( pos + 1 ) & mask;
   }
   return false;
  }
