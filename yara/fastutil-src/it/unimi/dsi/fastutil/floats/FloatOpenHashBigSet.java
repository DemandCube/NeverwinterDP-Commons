...
public class FloatOpenHashBigSet extends AbstractFloatSet implements java.io.Serializable, Cloneable, Hash, Size64 {
 ...
 public boolean add( final float k ) {
  final long h = it.unimi.dsi.fastutil.HashCommon.murmurHash3( (long)( it.unimi.dsi.fastutil.HashCommon.float2int(k) ^ mask ) );
  // The starting point.
  int displ = (int)( h & segmentMask );
  int base = (int)( ( h & mask ) >>> BigArrays.SEGMENT_SHIFT );
  // There's always an unused entry.
  while( used[ base ][ displ ] ) {
   if ( ( Float.floatToIntBits(key[ base ][ displ ]) == Float.floatToIntBits(k) ) ) return false;
 ...
 protected final long shiftKeys( long pos ) {
  ...
  for(;;) {
   ...
   while( BooleanBigArrays.get( used, pos ) ) {
    slot = it.unimi.dsi.fastutil.HashCommon.murmurHash3( (long)( it.unimi.dsi.fastutil.HashCommon.float2int(FloatBigArrays.get( key, pos )) ^ mask ) ) & mask;
 ...
 public boolean remove( final float k ) {
  ...
  // There's always an unused entry.
  while( used[ base ][ displ ] ) {
   if ( ( Float.floatToIntBits(key[ base ][ displ ]) == Float.floatToIntBits(k) ) ) {
    size--;
 ...
 public boolean contains( final float k ) {
  ...
  // There's always an unused entry.
  while( used[ base ][ displ ] ) {
   if ( ( Float.floatToIntBits(key[ base ][ displ ]) == Float.floatToIntBits(k) ) ) return true;
   base = ( base + ( ( displ = ( displ + 1 ) & segmentMask ) == 0 ? 1 : 0 ) ) & baseMask;
  }
  return false;
 }
 ...
 protected void rehash( final long newN ) {
  ...
  for( long i = size; i-- != 0; ) {
   ...
   k = key[ base ][ displ ];
   h = it.unimi.dsi.fastutil.HashCommon.murmurHash3( (long)( it.unimi.dsi.fastutil.HashCommon.float2int(k) ^ mask ) );
   // The starting point.
