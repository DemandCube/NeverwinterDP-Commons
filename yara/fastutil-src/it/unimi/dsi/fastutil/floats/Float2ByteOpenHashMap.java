...
public class Float2ByteOpenHashMap extends AbstractFloat2ByteMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Byte get( final Float ok ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits( k) ) ) return (Byte.valueOf(value[ pos ]));
   pos = ( pos + 1 ) & mask;
  }
  return (null);
 }
 ...
 private final class MapEntrySet extends AbstractObjectSet<Float2ByteMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   // There's always an unused entry.
   while( used[ pos ] ) {
    if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits(k) ) ) return ( (value[ pos ]) == (((e.getValue()).byteValue())) );
    pos = ( pos + 1 ) & mask;
   }
   return false;
  }
