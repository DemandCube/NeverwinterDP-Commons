...
public class Float2BooleanOpenHashMap extends AbstractFloat2BooleanMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public boolean put(final float k, final boolean v) {
  // The starting point.
  int pos = it.unimi.dsi.fastutil.HashCommon.murmurHash3( it.unimi.dsi.fastutil.HashCommon.float2int(k) ^ mask ) & mask;
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits(k) ) ) {
 ...
 protected final int shiftKeys( int pos ) {
  ...
  for(;;) {
   pos = ( ( last = pos ) + 1 ) & mask;
   while( used[ pos ] ) {
    slot = it.unimi.dsi.fastutil.HashCommon.murmurHash3( it.unimi.dsi.fastutil.HashCommon.float2int(key[ pos ]) ^ mask ) & mask;
 ...
 public Boolean get( final Float ok ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits( k) ) ) return (Boolean.valueOf(value[ pos ]));
   pos = ( pos + 1 ) & mask;
  }
  return (null);
 }
 ...
 public boolean get( final float k ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits(k) ) ) return value[ pos ];
   pos = ( pos + 1 ) & mask;
  }
  return defRetValue;
 }
 ...
 public boolean containsKey( final float k ) {
  ...
  // There's always an unused entry.
  while( used[ pos ] ) {
   if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits(k) ) ) return true;
   pos = ( pos + 1 ) & mask;
  }
  return false;
 }
 ...
 private class MapIterator {
  ...
  public int nextEntry() {
   ...
   if ( pos < 0 ) {
    ...
    // There's always an unused entry.
    while( used[ pos ] ) {
     if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits(k) ) ) return pos;
     pos = ( pos + 1 ) & mask;
    }
   }
   last = pos;
 ...
 private final class MapEntrySet extends AbstractObjectSet<Float2BooleanMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   // There's always an unused entry.
   while( used[ pos ] ) {
    if ( ( Float.floatToIntBits(key[ pos ]) == Float.floatToIntBits(k) ) ) return ( (value[ pos ]) == (((e.getValue()).booleanValue())) );
    pos = ( pos + 1 ) & mask;
   }
   return false;
  }
 ...
 protected void rehash( final int newN ) {
  ...
  for( int j = size; j-- != 0; ) {
   while( ! used[ i ] ) i++;
   k = key[ i ];
   pos = it.unimi.dsi.fastutil.HashCommon.murmurHash3( it.unimi.dsi.fastutil.HashCommon.float2int(k) ^ mask ) & mask;
